...
// Últimas mensagens (com nome da instância e status). Escopo por papel.
export async function getRecentMessages(input: {
  ...
  // MEMBER sem instância → nada.
  let instanceFilter: { in: string[] } | undefined
  if (ownerUserId) {
    const ids = await visibleInstanceIds(apiClientId, ownerUserId)
    if (ids.length === 0) return []
    instanceFilter = { in: ids }
  }
  const messages = await prisma.message.findMany({
    where: { apiClientId, ...(instanceFilter ? { instanceId: instanceFilter } : {}) },
